...
$ ->
  ...
  Skull = require 'skull'
  class StepItem extends Skull.Model
    initialize: ->
      @bind 'remove', => @view.remove() if @view
    
  class StepCollection extends Skull.Collection
    url: '/steps'
    model: StepItem

  class StepItemView extends Backbone.View
    
    template: _.template $('#step-item').html()
    tagName: 'section'
    events:
